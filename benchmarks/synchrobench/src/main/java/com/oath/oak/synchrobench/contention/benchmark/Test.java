...
public class Test {
    ...
    private void parseCommandLineParameters(String[] args) throws Exception {
        ...
        if (Parameters.range != 2 * Parameters.size) {
            System.err
                    .println("Note that the value range is not twice the initial size, thus the size expectation " +
                            "varies at runtime.");
        }
    }
        ...
        switch (benchType) {
            case OAKMAP:
                if (oakBench instanceof MaintenanceAlg) {
                    System.out.println("  #nodes (inc. deleted): \t" + ((MaintenanceAlg) oakBench).numNodes());
                }
                break;
        }
